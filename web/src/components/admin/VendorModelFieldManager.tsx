...
interface VendorModelFieldManagerProps {
  open: boolean
...
export function VendorModelFieldManager({ open, onOpenChange, vendorModelId, modelId, vendorName, modelName }: VendorModelFieldManagerProps) {
  ...
  const loadFields = async () => {
    setLoading(true)
    try {
      const res = await getVendorModelFields(vendorModelId)
  ...
  const handleDelete = async (id: number) => {
    try { await deleteVMField(vendorModelId, id); loadFields() } catch { /* ignore */ }
  }
        ...
        <div className='flex items-center justify-between border-b border-border/40 px-6 py-4'>
          <div>
            ...
            <p className='text-muted-foreground mt-0.5 text-sm'>
              ...
              {' · '}
              模型：<span className='text-foreground font-medium'>{modelName}</span>
            </p>
          </div>
          ...
          <table className='w-full text-sm'>
            ...
            <tbody className='divide-border/40 divide-y'>
              {loading ? (
                ...
                <tr><td colSpan={7} className='text-muted-foreground px-4 py-12 text-center text-sm'>暂无字段定义，可从模型同步或手动添加</td></tr>
              ) : (
                fields.map((f) => (
